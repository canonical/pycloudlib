...
class EC2Instance(BaseInstance):
    ...
    def add_network_interface(
        ...
        *,
        ...
        ipv4_public_ip_count: Optional[int] = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            ipv4_public_ip_count: number of public ips associated to private
                ips.
            associate_public_ip: created boto3 instance object
        ...
        if ipv4_public_ip_count is not None:
            if (
                ipv4_address_count is None
                or ipv4_address_count < ipv4_public_ip_count
            ):
                raise PycloudlibError(
                    f"Invalid combination of"
                    f" `ipv4_address_count={ipv4_public_ip_count}` and"
                    f" `ipv4_address_count={ipv4_address_count}`"
                )

        if ipv4_address_count is not None and ipv4_address_count > 1:
            ...
            if ipv4_public_ip_count is not None and ipv4_address_count > 1:
                nics_resp = self._client.describe_network_interfaces(
                    NetworkInterfaceIds=[interface_id]
                )
                _check_response(nics_resp)
                priv_ips = [
                    nics_resp["NetworkInterfaces"][0]["PrivateIpAddress"]
                ]
                priv_ips.extend(
                    (
                        x["PrivateIpAddress"]
                ...
                for i in range(ipv4_address_count):
                    allo = self._client.allocate_address(Domain="vpc")
                    _check_response(allo)
    ...
    def _release_address(self, private_ip: str):
        ...
        for association in self._find_nic_associations(nics_resp):
            try:
                ...
                self._log.debug(
                    "Error deleting Public IP associated from NIC with IP"
                    " %s: %s",
                    private_ip,
                    str(ex),
                )
