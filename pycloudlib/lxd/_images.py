...
class ImageLocator:
    """Class concern with LXD images' operations."""
    ...
    def find_last_fingerprint(
        ...
        """
        remote = cls._REMOTE_DAILY if daily else cls._REMOTE_RELEASE
        remote += ":"
        base_filters = (
            ("architecture", arch),
            ("release", release),
        ...
        find_images = functools.partial(cls._find_images, remote)
    ...
    @classmethod
    def find_release(cls, image_id: str) -> Optional[str]:
        ...
        """
        release = None
        images = cls._find_images(image_id)
        if not images:
            return release
        image_info = images[0]
    ...
    @classmethod
    def find_image_serial(cls, image_id: str) -> Optional[str]:
        ...
        """
        if ":" not in image_id:
            remote = cls._REMOTE_DAILY
        else:
            remote, image_id = image_id.split(":", 1)
        filters = (("fingerprint", image_id),)
        image = cls._find_images(remote, filters)
        if not image:
            return None
        return image[0]["properties"]["serial"]

    @classmethod
    def _normalize_remote(cls, remote: Optional[str] = None) -> str:
        if not remote:
            remote = cls._REMOTE_DAILY
        if ":" not in remote:
            remote += ":"
    ...
    @classmethod
    def _find_images(
        cls, remote: str, filters: Optional[Sequence[Tuple[str, str]]] = None
    ) -> List[dict]:
        """Find the latest image for a given release.

        Args:
            release: Ubuntu release to look for
            arch: string, architecture to use

        Returns:
            list of dictionaries with image's info
        """
        remote = cls._normalize_remote(remote)
        cmd = [
            "lxc",
            "image",
            "list",
