...
@mock.patch("pycloudlib.qemu.instance.QmpConnection", side_effect=AssertionError)
...
@mock.patch("pycloudlib.qemu.instance.Path")
@mock.patch("pycloudlib.qemu.instance.QemuInstance._setup_qmp", return_value=None)
...
def test_delete_no_handle(m_rmtree, m_qmp, m_path):
    ...
    assert str(errors[0]) == "No QMP connection or process handle. Manual cleanup required"
    m_rmtree.assert_called_once_with(instance.instance_dir)
